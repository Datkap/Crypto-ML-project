...
def collect_data(num_of_cryptos: int, fiat_currency="USD", data_folder_path="Quotes"):
    """Combines multiple function in order to collect data about cryptocurrency quotations, Fear and Greed Index score
    and put them in separate data files."""
    quotes = get_crypto(num_of_cryptos, fiat_currency)
    fng = get_fear_and_greed()
    crypto_df = combine_quotes_and_fng(quotes, fng)
    save_data(crypto_df, data_folder_path)
